...
function TransactionTableDistributer() {
    ...
    const debitExportExcel = async () => {
        if (window.confirm('Are you sure you want to export Excel ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}mfProductrouter/exportExcelForDistributorDebitTransactionList?startDate=${state[0].startDate}&endDate=${state[0].endDate}` : `${BACKEND_BASE_URL}mfProductrouter/exportExcelForDistributorDebitTransactionList?startDate=${''}&endDate=${''}`,
                method: 'GET',
    ...
    const debitExportPdf = async () => {
        if (window.confirm('Are you sure you want to export Pdf ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}mfProductrouter/exportPdfForDistributorDebitTransactionList?startDate=${state[0].startDate}&endDate=${state[0].endDate}` : `${BACKEND_BASE_URL}mfProductrouter/exportPdfForDistributorDebitTransactionList?startDate=${''}&endDate=${''}`,
                method: 'GET',
                headers: { Authorization: `Bearer ${userInfo.token}` },
                responseType: 'blob', // important
            }).then((response) => {
        ...
        <div className='productListContainer'>
            ...
            {tab === 2 || tab === '2' ?
                <div className='mt-6 grid grid-cols-4 gap-6'>
                    <CountCard color={'blue'} count={debitCount && debitCount.totalSell ? debitCount.totalSell : 0} desc={'Total Sold'} unitDesc={0} />
                    ...
                    <CountCard color={'yellow'} count={debitCount && debitCount.totalGain ? debitCount.totalGain : 0} desc={'Recived Debit'} unitDesc={0} />
...
export default TransactionTableDistributer;
