...
function ProductListTableStockIn(props) {
    ...
    const productExportExcel = async () => {
        if (window.confirm('Are you sure you want to export Excel ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}rawMaterialrouter/exportExcelSheetForRawMaterialTable?startDate=${state[0].startDate}&endDate=${state[0].endDate}&rawMaterialCategoryId=${props.id}` : `${BACKEND_BASE_URL}rawMaterialrouter/exportExcelSheetForRawMaterialTable?startDate=${''}&endDate=${''}&rawMaterialCategoryId=${props.id}`,
                method: 'GET',
    ...
    const productExportPdf = async () => {
        if (window.confirm('Are you sure you want to export Pdf ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}rawMaterialrouter/exportPdfForAllRawMaterialData?startDate=${state[0].startDate}&endDate=${state[0].endDate}&rawMaterialCategoryId=${props.id}` : `${BACKEND_BASE_URL}rawMaterialrouter/exportPdfForAllRawMaterialData?startDate=${''}&endDate=${''}&rawMaterialCategoryId=${props.id}`,
                method: 'GET',
                headers: { Authorization: `Bearer ${userInfo.token}` },
                responseType: 'blob', // important
            }).then((response) => {
    ...
    const productExportPdfByStatus = async () => {
        if (window.confirm('Are you sure you want to export Pdf ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}rawMaterialrouter/exportPdfForAllRawMaterialData?startDate=${state[0].startDate}&endDate=${state[0].endDate}&rawMaterialStatus=${tab}&rawMaterialCategoryId=${props.id}` : `${BACKEND_BASE_URL}rawMaterialrouter/exportPdfForAllRawMaterialData?startDate=${''}&endDate=${''}&rawMaterialStatus=${tab}&rawMaterialCategoryId=${props.id}`,
                method: 'GET',
                headers: { Authorization: `Bearer ${userInfo.token}` },
                responseType: 'blob', // important
            }).then((response) => {
