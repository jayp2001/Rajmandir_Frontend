...
function MaterialListTable() {
    ...
    const handleDeleteProduct = async (id) => {
        const password = window.prompt("Are you sure you want to delete Material?... Enter Password to delete")
        if (password) {
    ...
    const productExportExcel = async () => {
        if (window.confirm('Are you sure you want to export Excel ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}rawMaterialrouter/exportExcelSheetForRawMaterialTable?startDate=${state[0].startDate}&endDate=${state[0].endDate}` : `${BACKEND_BASE_URL}rawMaterialrouter/exportExcelSheetForRawMaterialTable?startDate=${''}&endDate=${''}`,
                method: 'GET',
    ...
    const productExportPdf = async () => {
        if (window.confirm('Are you sure you want to export Pdf ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}rawMaterialrouter/exportPdfForAllRawMaterialData?startDate=${state[0].startDate}&endDate=${state[0].endDate}` : `${BACKEND_BASE_URL}rawMaterialrouter/exportPdfForAllRawMaterialData?startDate=${''}&endDate=${''}`,
                method: 'GET',
                headers: { Authorization: `Bearer ${userInfo.token}` },
                responseType: 'blob', // important
            }).then((response) => {
    ...
    const productExportPdfByStatus = async () => {
        if (window.confirm('Are you sure you want to export Pdf ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}rawMaterialrouter/exportPdfForAllRawMaterialData?startDate=${state[0].startDate}&endDate=${state[0].endDate}&rawMaterialStatus=${tab}` : `${BACKEND_BASE_URL}rawMaterialrouter/exportPdfForAllRawMaterialData?startDate=${''}&endDate=${''}&rawMaterialStatus=${tab}`,
                method: 'GET',
                headers: { Authorization: `Bearer ${userInfo.token}` },
                responseType: 'blob', // important
            }).then((response) => {
                ...
                const name = 'RawMaterial_Pdf_' + SN + new Date().toLocaleDateString() + '.pdf'
                link.href = href;
                link.setAttribute('download', name); //or any other extension
                document.body.appendChild(link);
                link.click();
    ...
    const handleViewDetail = (id, name, unit, remainingQty, status) => {
        navigate(`/material/materialDetails/${id}/${name}/${unit}/${remainingQty}/${status == 0 || !status ? false : true}`)
    }
    if (loading) {
        console.log('>>>>??')
                        ...
                        <div className='h-full grid grid-cols-12'>
                            ...
                            <div className=' grid col-span-2 col-start-11 pr-3 flex h-full'>
                                <div className='self-center justify-self-end'>
                                    <button className='addProductBtn' onClick={handleOpen}>Add Material</button>
                                </div>
                            </div>
                        </div>
                                            ...
                                            {allData?.map((row, index) => (
                                                totalRows !== 0 ?
                                                    ...
                                                    >
                                                        ...
                                                        <TableCell align="left" onClick={() => handleViewDetail(row.rawMaterialId, row.rawMaterialName, row.minRawMaterialUnit, row.remainingStock, row.isSupplyBranch)}>{row.minRawMaterialQty}</TableCell>
                                            ...
                                            {allData?.map((row, index) => (
                                                totalRows !== 0 ?
                                                    ...
                                                    >
                                                        ...
                                                        {/* </Tooltip> */}
                                                        <TableCell align="left" onClick={() => handleViewDetail(row.rawMaterialId, row.rawMaterialName, row.minRawMaterialUnit, row.remainingStock, row.isSupplyBranch)}>{row.purchese} </TableCell>
                                                        ...
                                                        <TableCell align="left" onClick={() => handleViewDetail(row.rawMaterialId, row.rawMaterialName, row.minRawMaterialUnit, row.remainingStock, row.isSupplyBranch)}><div className='Comment'>{row.totalUsed} </div></TableCell>
