...
function FactoryDistributerDetail() {
    let { id } = useParams();
    ...
    const stockInExportExcel = async () => {
        if (window.confirm('Are you sure you want to export Excel ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}mfProductrouter/exportExcelForDistributorCashAndDebit?startDate=${state[0].startDate}&endDate=${state[0].endDate}&distributorId=${id}&payType=${tabStockIn}` : `${BACKEND_BASE_URL}mfProductrouter/exportExcelForDistributorCashAndDebit?startDate=${''}&endDate=${''}&distributorId=${id}&payType=${tabStockIn}`,
                method: 'GET',
                headers: { Authorization: `Bearer ${userInfo.token}` },
                responseType: 'blob', // important
            }).then((response) => {
                ...
                // create "a" HTML element with href to file & click
                const link = document.createElement('a');
                const name = 'Sold_' + new Date().toLocaleDateString() + '.xlsx'
                link.href = href;
                link.setAttribute('download', name); //or any other extension
                document.body.appendChild(link);
                link.click();
    ...
    const stockInExportPdf = async () => {
        if (window.confirm('Are you sure you want to export Pdf ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}mfProductrouter/exportPdfForDistributorCashAndDebit?startDate=${state[0].startDate}&endDate=${state[0].endDate}&distributorId=${id}&payType=${tabStockIn}` : `${BACKEND_BASE_URL}mfProductrouter/exportPdfForDistributorCashAndDebit?startDate=${''}&endDate=${''}&distributorId=${id}&payType=${tabStockIn}`,
                method: 'GET',
                headers: { Authorization: `Bearer ${userInfo.token}` },
                responseType: 'blob', // important
            }).then((response) => {
                ...
                // create "a" HTML element with href to file & click
                const link = document.createElement('a');
                const name = 'Sold_' + new Date().toLocaleDateString() + '.pdf'
                link.href = href;
                link.setAttribute('download', name); //or any other extension
                document.body.appendChild(link);
                link.click();
    ...
    const allProductExportPdf = async () => {
        if (window.confirm('Are you sure you want to export Pdf ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}mfProductrouter/exportPdfForAllProductDetailsByDistributorId?startDate=${state[0].startDate}&endDate=${state[0].endDate}&distributorId=${id}&payType=${tabStockIn}` : `${BACKEND_BASE_URL}mfProductrouter/exportPdfForAllProductDetailsByDistributorId?startDate=${''}&endDate=${''}&distributorId=${id}&payType=${tabStockIn}`,
                method: 'GET',
                headers: { Authorization: `Bearer ${userInfo.token}` },
                responseType: 'blob', // important
            }).then((response) => {
    ...
    const transactionExportPdf = async () => {
        if (window.confirm('Are you sure you want to export Pdf ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}mfProductrouter/exportPdfForDistributorDebitTransactionList?startDate=${state[0].startDate}&endDate=${state[0].endDate}&distributorId=${id}` : `${BACKEND_BASE_URL}mfProductrouter/exportPdfForDistributorDebitTransactionList?startDate=${''}&endDate=${''}&distributorId=${id}`,
                method: 'GET',
                headers: { Authorization: `Bearer ${userInfo.token}` },
                responseType: 'blob', // important
            }).then((response) => {
    ...
    const allProductExportExcel = async () => {
        if (window.confirm('Are you sure you want to export Excel ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}mfProductrouter/exportExcelSheetForAllProductDetailsByDistributorId?startDate=${state[0].startDate}&endDate=${state[0].endDate}&distributorId=${id}&payType=${tabStockIn}` : `${BACKEND_BASE_URL}mfProductrouter/exportExcelSheetForAllProductDetailsByDistributorId?startDate=${''}&endDate=${''}&distributorId=${id}&payType=${tabStockIn}`,
                method: 'GET',
                headers: { Authorization: `Bearer ${userInfo.token}` },
                responseType: 'blob', // important
            }).then((response) => {
    ...
    const transactionExportExcel = async () => {
        if (window.confirm('Are you sure you want to export Excel ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}mfProductrouter/exportExcelForDistributorDebitTransactionList?startDate=${state[0].startDate}&endDate=${state[0].endDate}&distributorId=${id}` : `${BACKEND_BASE_URL}mfProductrouter/exportExcelForDistributorDebitTransactionList?startDate=${''}&endDate=${''}&distributorId=${id}`,
                method: 'GET',
                headers: { Authorization: `Bearer ${userInfo.token}` },
                responseType: 'blob', // important
            }).then((response) => {
                                        ...
                                        {stockInData?.map((row, index) => (
                                            totalRows !== 0 ?
                                                ...
                                                >
                                                    ...
                                                    <TableCell align="right" >{parseFloat(row.costPrice ? row.costPrice : 0).toLocaleString('en-IN')}</TableCell>
                                                    <TableCell align="right" >{parseFloat(row.sellAmount && row.costPrice ? row.sellAmount - row.costPrice : 0).toLocaleString('en-IN')}</TableCell>
                                    ...
                                    <Table sx={{ minWidth: 650 }} stickyHeader aria-label="sticky table">
                                        <TableHead>
                                            <TableRow>
                                                ...
                                                <TableCell align="left">Production Cost</TableCell>
                                            ...
                                            {productTable?.map((row, index) => (
                                                totalRowsProduct !== 0 ?
                                                    ...
                                                    >
                                                        ...
                                                        <TableCell align="left" >{parseFloat(row.cost ? row.cost : 0).toLocaleString('en-IN')}</TableCell>
                                                        <TableCell align="left" >{parseFloat(row.totalSellPrice && row.cost ? row.totalSellPrice - row.cost : 0).toLocaleString('en-IN')}</TableCell>
                                                    </TableRow> :
                                                    <TableRow
                                                        key={row.userId}
                                                        sx={{ '&:last-child td, &:last-child th': { border: 0 } }}
...
export default FactoryDistributerDetail;