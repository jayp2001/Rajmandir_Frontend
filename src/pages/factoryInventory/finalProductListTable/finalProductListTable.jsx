...
function FinalProductListTable() {
    const regex = /^\d*(?:\.\d*)?$/;
    ...
    const deleteRecipe = async (id) => {
        await axios.delete(`${BACKEND_BASE_URL}mfProductrouter/removeRecipeeData?mfProductId=${id}`, config)
            .then((res) => {
                setSuccess(true)
                setPage(0);
                setTab('')
    ...
    const handleDeleteRecipe = async (id) => {
        const password = window.prompt("Are you sure you want to delete Recipe?... Enter Password to delete")
        if (password) {
            await axios.post(`${BACKEND_BASE_URL}userrouter/chkPassword`, { "userPassword": password }, config)
                .then(async (res) => {
                    deleteRecipe(id);
                                            ...
                                            {allData?.map((row, index) => (
                                                totalRows !== 0 ?
                                                    ...
                                                    >
                                                        ...
                                                        {/* <TableCell align="left" onClick={() => handleViewDetail(row.mfProductId, row.mfProductName, row.minMfProductUnit, row.remainingStock)}>{row.lastUpdatedStockInDate}</TableCell> */}
                                                        <TableCell align="right">
                                                            <Menutemp mfProductId={row.mfProductId} data={row} handleOpenAddRecipe={handleOpenAddRecipe} handleOpenEditRecipe={handleOpenEditRecipe} handleOpenStockOut={handleOpenStockOut} handleOpenStockIn={handleOpenStockIn} handleDeleteProduct={handleDeleteProduct} handleDeleteRecipe={handleDeleteRecipe} handleEditClick={handleEditClick} />
                                                        </TableCell>
                                                    </TableRow> :
                                                    <TableRow
                                                        key={row.userId}
                                                ...
                                                <div className={`grid grid-cols-12 gap-6`}>
                                                    ...
                                                    <div className='col-span-5'>
                                                        <TextField
                                                            ...
                                                            helperText={recipeErrorStockIn[index] && recipeErrorStockIn[index].usedMaterial ? `Remaining Qty ${data.remainQty + ' ' + data.rmUnit}` : `Remaining Qty ${data.remainQty + ' ' + data.rmUnit}`}
                                                            name="usedMaterial"
                                                            id="outlined-required"
                                                            label="Qty"
                                                            // endAdornment={<InputAdornment position="end">kg</InputAdornment>}
...
export default FinalProductListTable;
