...
function StockInOutFactory() {
    ...
    const [editStockInId, setEditStockInId] = React.useState('')
    ...
    const fillStockInEdit = async (id) => {
        await axios.get(`${BACKEND_BASE_URL}mfProductrouter/fillMfProductStockInData?mfStockInId=${id.mfStockInId}`, config)
            .then((res) => {
                setEditStockInId(id.mfStockInId);
                setStockInFormData((perv) => ({
                    ...perv,
    ...
    const fillRecipeDataForStockIn = async (id, qty, unit, batchQty) => {
        await axios.get(isEdit ? `${BACKEND_BASE_URL}mfProductrouter/fillRecipeeDataByQty?mfProductId=${id}&qty=${qty}&unit=${unit}&batchQty=${batchQty}&mfStockInId=${editStockInId}` : `${BACKEND_BASE_URL}mfProductrouter/fillRecipeeDataByQty?mfProductId=${id}&qty=${qty}&unit=${unit}&batchQty=${batchQty}`, config)
            .then((res) => {
    ...
    const fillRecipeDataForStockInBatchWise = async (id, qty) => {
        await axios.get(isEdit ? `${BACKEND_BASE_URL}mfProductrouter/fillRecipeeDataByBatch?mfProductId=${id}&batchQty=${qty}&mfStockInId=${editStockInId}` : `${BACKEND_BASE_URL}mfProductrouter/fillRecipeeDataByBatch?mfProductId=${id}&batchQty=${qty}`, config)
            .then((res) => {
                setRecipeStockIn(res.data.recipeMaterial);
                setRecipeExpenseStockIn(res.data.otherExpense);
                setRecipeProductStockIn(res.data.produceProductdata);
    ...
    const getStockOutData = async () => {
        await axios.get(`${BACKEND_BASE_URL}mfProductrouter/getMfStockOutList?page=${1}&numPerPage=${5}`, config)
            .then((res) => {
                setStockOutData(res.data.rows);
                setTotalRowsOut(res.data.numRows);
            })
    ...
    const getStockOutDataByFilter = async () => {
        await axios.get(`${BACKEND_BASE_URL}mfProductrouter/getMfStockOutList?startDate=${state[0].startDate}&endDate=${state[0].endDate}&page=${1}&numPerPage=${5}`, config)
            .then((res) => {
                setStockOutData(res.data.rows);
                setTotalRowsOut(res.data.numRows);
            })
    ...
    const stockInExportExcel = async () => {
        if (window.confirm('Are you sure you want to export Excel ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}mfProductrouter/exportExcelSheetForMfStockIn?startDate=${state[0].startDate}&endDate=${state[0].endDate}` : `${BACKEND_BASE_URL}mfProductrouter/exportExcelSheetForMfStockIn?startDate=${''}&endDate=${''}`,
                method: 'GET',
    ...
    const stockInExportPdf = async () => {
        if (window.confirm('Are you sure you want to export Pdf ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}mfProductrouter/exportPdfForMfStockIn?startDate=${state[0].startDate}&endDate=${state[0].endDate}` : `${BACKEND_BASE_URL}mfProductrouter/exportPdfForMfStockIn?startDate=${''}&endDate=${''}`,
                method: 'GET',
                headers: { Authorization: `Bearer ${userInfo.token}` },
                responseType: 'blob', // important
            }).then((response) => {
    ...
    const stockOutExportExcel = async () => {
        if (window.confirm('Are you sure you want to export Excel ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}mfProductrouter/exportExcelSheetForMfStockOut?startDate=${state[0].startDate}&endDate=${state[0].endDate}` : `${BACKEND_BASE_URL}mfProductrouter/exportExcelSheetForMfStockOut?startDate=${''}&endDate=${''}`,
                method: 'GET',
                headers: { Authorization: `Bearer ${userInfo.token}` },
                responseType: 'blob', // important
            }).then((response) => {
    ...
    const stockOutExportPdf = async () => {
        if (window.confirm('Are you sure you want to export Pdf ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}mfProductrouter/exportPdfForMfStockOut?startDate=${state[0].startDate}&endDate=${state[0].endDate}` : `${BACKEND_BASE_URL}mfProductrouter/exportPdfForMfStockOut?startDate=${''}&endDate=${''}`,
                method: 'GET',
                headers: { Authorization: `Bearer ${userInfo.token}` },
                responseType: 'blob', // important
            }).then((response) => {
                                            ...
                                            {stockInData?.map((row, index) => (
                                                totalRows !== 0 ?
                                                    ...
                                                    >
                                                        ...
                                                        <TableCell align="right" onClick={() => handleAccordionOpenOnView(row)}>{parseFloat(row.mfProductPrice ? row.mfProductPrice : 0).toLocaleString('en-IN')}</TableCell>
                                                        <TableCell align="right" onClick={() => handleAccordionOpenOnView(row)}>{parseFloat(row.totalPrice ? row.totalPrice : 0).toLocaleString('en-IN')}</TableCell>
            ...
            >
                <Box sx={style}>
                    ...
                    <div className='mt-6 grid grid-cols-12 gap-6'>
                        ...
                        {stockInFormData.isAuto &&
                            <div className='col-span-3'>
                                <TextField
                                    ...
                                    label="Batch Qty"
                                    fullWidth
                                    disabled={isView ? true : stockInFormData.mfProductName ? false : true}
                        ...
                        {
                            isView ? <div className='col-span-3'>
                                <TextField
                                    onBlur={(e) => {
                                        if (e.target.value < 0) {
                                            setStockInFormDataError((perv) => ({
                    ...
                    {
                        (stockInFormData.isAuto && !isView && (recipeStockIn && recipeStockIn.length > 0 || recipeExpenseStockIn && recipeExpenseStockIn.length > 0 || recipeProductStockIn && recipeProductStockIn.length > 0)) ?
                            <>
                                <div className='mt-4'>
                                                    ...
                                                    <div className={`grid grid-cols-12 gap-6`}>
                                                        ...
                                                        <div className='col-span-3'>
                                                            <TextField
                                                                type='text'
                                                                onChange={(e) => onChangeRecipeStockIn(e, index)}
                                                                value={data.rmStockOutPrice ? data.rmStockOutPrice : ''}
                                                                name="usedMaterial"
                                                                id="outlined-required"
                                                                label="Cost"
                                                                disabled={isView}
                                                    ...
                                                    <div className={`grid grid-cols-12 gap-6`}>
                                                        ...
                                                        <div className='col-span-3'>
                                                            <TextField
                                                                type='text'
                                                                onChange={(e) => onChangeRecipeStockIn(e, index)}
                                                                value={data.mfProductOutPrice ? data.mfProductOutPrice : ''}
                                                                name="usedMaterial"
                                                                id="outlined-required"
                                                                label="Cost"
                                                                disabled={isView}
...
export default StockInOutFactory;