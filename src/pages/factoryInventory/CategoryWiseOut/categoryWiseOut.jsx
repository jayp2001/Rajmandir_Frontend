...
function CategoryWiseOut() {
    ...
    const getDebitCountsByFilterByCategory = async (outId, branchId) => {
        await axios.get(filter ? `${BACKEND_BASE_URL}mfProductrouter/getStaticsOutCategoryWiseMfProductData?outCategoryId=${outId}&branchId=${branchId}&startDate=${state[0].startDate}&endDate=${state[0].endDate}` : `${BACKEND_BASE_URL}mfProductrouter/getStaticsOutCategoryWiseMfProductData?outCategoryId=${outId}&branchId=${branchId}`, config)
            .then((res) => {
                setCounts(res.data);
            })
            .catch((error) => {
    ...
    return (
        <div className='productListContainer'>
            ...
            <div className='mt-6 grid grid-cols-4 gap-6'>
                <CountCard color={'blue'} count={counts && counts.sellAmt ? counts.sellAmt : 0} desc={'Total Sold'} unitDesc={0} />
                <CountCard color={'green'} count={counts && counts.costPrice ? counts.costPrice : 0} desc={'Total Production Cost'} unitDesc={0} />
                <CountCard color={'yellow'} count={counts && counts.profit ? counts.profit : 0} desc={'Profit'} unitDesc={0} />
                {/* <CountCard color={'pink'} count={counts && counts.remainingAmount ? counts.remainingAmount : 0} desc={'Total Remaining'} unitDesc={0} /> */}
            </div>
                                ...
                                <FormControl style={{ minWidth: '100%', maxWidth: '100%' }}>
                                    ...
                                    <Select
                                        ...
                                        onChange={(e) => {
                                            ...
                                            filter ? getDebitCountsByFilterByCategory(e.target.value, '') : getDebitCountsByFilterByCategory(e.target.value, '')
                                        }}
                                    >
                                ...
                                <Table sx={{ minWidth: 650 }} stickyHeader aria-label="sticky table">
                                    <TableHead >
                                        <TableRow>
                                            ...
                                            <TableCell align="left">Profit In Percentage</TableCell>
                                        ...
                                        {allData?.map((row, index) => (
                                            totalRows !== 0 ?
                                                ...
                                                >
                                                    ...
                                                    <TableCell align="left" >{parseFloat(row.profit ? row.profit : 0).toLocaleString('en-IN')}</TableCell>
                                                    <TableCell align="left" >{row.sellAmt != 0 ? ((parseFloat(row.costPrice && row.profit ? row.profit : 0) * 100) / parseFloat(row.sellAmt ? row.sellAmt : 1)).toFixed(2) : -100} %</TableCell>
...
export default CategoryWiseOut;
