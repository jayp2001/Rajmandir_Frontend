                            ...
                            >
                                <table className="w-full text-xs">
                                    <thead className="bg-gray-100 sticky top-0 z-10">
                                        <tr>
                                            <th className="text-left py-2.5 pl-4 pr-2 font-semibold border-b-2 border-gray-300">Particulars</th>
                                            <th className="text-center py-2.5 px-3 font-semibold border-b-2 border-gray-300">Qty</th>
                                            <th className="text-center py-2.5 px-3 font-semibold border-b-2 border-gray-300">Price</th>
                                            <th className="text-right py-2.5 pl-3 pr-4 font-semibold border-b-2 border-gray-300">Amt.</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {infoPopUpData?.itemData?.map((item, index) => (
                                            <tr key={index} className="border-b border-gray-100 hover:bg-blue-50">
                                                <td className="py-2 pl-4 pr-2 font-semibold text-gray-800">
                                                    {item.itemName}{item.unit && item.unit.toLowerCase() !== "no" ? ` (${item.unit})` : ""}
                                                    {item.addons && Object.keys(item.addons).length > 0 && (
                                                        <div className="text-xs mt-1 mb-1">
                                                            <ul className="m-0 pl-4">
                                                                {Object.values(item.addons).map((addon, addonIndex) => (
                                                                    <li key={addonIndex} className="text-xs mb-0.5">
                                                                        {addon.addonsName} {addon.addonPrice ? `(${item.qty} X ${addon.addonPrice} = ${item.qty * addon.addonPrice}Rs.)` : ""}
                                                                    </li>
                                                                ))}
                                                            </ul>
                                                        </div>
                                                ...
                                                <td className="py-2 px-3 text-center text-gray-700">
                                                    {(parseFloat(item.itemPrice || 0) + (item.addons ? Object.values(item.addons).reduce((sum, addon) => sum + (parseFloat(addon.addonPrice || 0)), 0) : 0)).toFixed(2)}
                                                </td>
                                                <td className="py-2 pl-3 pr-4 text-right font-bold text-green-700">{parseFloat(item.price || 0).toFixed(2)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
