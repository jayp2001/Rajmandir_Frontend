...
function FactorySuppilerTableInOut() {
    ...
    const getData = async () => {
        ...
        await axios.get(`${BACKEND_BASE_URL}rawMaterialrouter/getFactorySupplierdata?page=${1}&numPerPage=${5}`, config)
            .then((res) => {
    ...
    const getDataOnPageChange = async (pageNum, rowPerPageNum) => {
        console.log("page get", page, rowsPerPage)
        await axios.get(`${BACKEND_BASE_URL}rawMaterialrouter/getFactorySupplierdata?page=${pageNum}&numPerPage=${rowPerPageNum}&searchWord=${searchWord}`, config)
            .then((res) => {
                setData(res.data.rows);
                setTotalRows(res.data.numRows);
            })
    ...
    const handleSuppilerOnClick = (id) => {
        navigate(`/inOut/material/suppilerDetails/${id}`)
    }
    const handleOpen = (row) => {
        // getCategoryList();
        setFormData((perv) => ({
            ...perv,
            rmSupplierId: row.rmSupplierId,
    ...
    const deleteData = async (id) => {
        await axios.delete(`${BACKEND_BASE_URL}rawMaterialrouter/removeFactorySupplierDetails?rmSupplierId=${id}`, config)
            .then((res) => {
                setPage(0);
                setSuccess(true)
                setRowsPerPage(5);
    ...
    const search = async (searchWord) => {
        await axios.get(`${BACKEND_BASE_URL}rawMaterialrouter/getFactorySupplierdata?page=${1}&numPerPage=${5}&searchWord=${searchWord}`, config)
            .then((res) => {
                setData(res.data.rows);
                setTotalRows(res.data.numRows);
            })
                    ...
                    <div className='userTableSubContainer'>
                        ...
                        <div className='grid grid-cols-12'>
                            ...
                            <div className='col-span-4 col-start-9 pr-8 flex justify-end'>
                                <button className='exportExcelBtn' onClick={() => { navigate(`/inOut/material/addSuppiler`) }}>Add Suppiler</button>
                            </div>
                        </div>
                                    ...
                                    <TableBody>
                                        {data?.map((row, index) => (
                                            totalRows !== 0 ?
                                                <TableRow
                                                    hover
                                                    key={row.rmSupplierId}
                                                    sx={{ '&:last-child td, &:last-child th': { border: 0 } }}
                                                    style={{ cursor: "pointer" }}
                                                    className='tableRow'
                                                >
                                                    <TableCell align="left" onClick={() => handleSuppilerOnClick(row.rmSupplierId)} >{(index + 1) + (page * rowsPerPage)}</TableCell>
                                                    <TableCell onClick={() => handleSuppilerOnClick(row.rmSupplierId)} component="th" scope="row">
                                                        {row.supplierNickName}
                                                    </TableCell>
                                                    <TableCell onClick={() => handleSuppilerOnClick(row.rmSupplierId)} align="left" >{row.supplierFirmName}</TableCell>
                                                    <TableCell onClick={() => handleSuppilerOnClick(row.rmSupplierId)} align="left" >{row.supplierName}</TableCell>
                                                    <TableCell onClick={() => handleSuppilerOnClick(row.rmSupplierId)} align="left" >{row.supplierPhoneNumber}</TableCell>
                                                    {/* <TableCell align="left" >{row.rightsName}</TableCell> */}
                                                    <TableCell onClick={() => handleSuppilerOnClick(row.rmSupplierId)} align="left" >{parseFloat(row.remainingAmount ? row.remainingAmount : 0).toLocaleString('en-IN')}</TableCell>
                                                    <TableCell align="right">
                                                        <Menutemp rmSupplierId={row.rmSupplierId} handleOpen={handleOpen} data={row} deleteSuppiler={handleDeleteSuppiler} />
                                                    </TableCell>
                                                </TableRow> :
                                                <TableRow
                                                    key={row.userId}
...
export default FactorySuppilerTableInOut;