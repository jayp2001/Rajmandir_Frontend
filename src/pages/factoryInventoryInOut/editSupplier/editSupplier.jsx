...
function EditFactorySupplierInOut() {
    let { id } = useParams();
    ...
    const [rawMaterialName, setProductName] = React.useState([]);
    ...
    const handleChange = (event, value) => {
        ...
        setFormData((pervState) => ({
            ...pervState,
            rawMaterialId: value.length > 0 ? res : [],
        }))
    };
    ...
    const getData = async () => {
        await axios.get(`${BACKEND_BASE_URL}rawMaterialrouter/ddlAllRawMaterialData`, config)
            .then(async (response) => {
                setProductList(response.data);
                await axios.get(`${BACKEND_BASE_URL}rawMaterialrouter/fillFactorySupplierDetails?rmSupplierId=${id}`, config)
                    .then((res) => {
    ...
    const reset = () => {
        navigate(`/inOut/material/supplierTable`)
    }
    const editSupplier = async () => {
        setLoading(true);
        await axios.post(`${BACKEND_BASE_URL}rawMaterialrouter/updateFactorySupplierDetails`, formData, config)
            .then((res) => {
                setLoading(false);
                setSuccess(true);
            })
                                ...
                                <div className='grid grid-cols-12 gap-6'>
                                    <div className="col-span-12">
                                        <Autocomplete
                                            ...
                                            defaultValue={null}
                                            value={rawMaterialName ? rawMaterialName : null}
                                            fullWidth
                                            id="checkboxes-tags-demo"
                                            options={productList ? productList : []}
                                            disableCloseOnSelect
                                            onChange={handleChange}
                                            error={formDataError.rawMaterialId}
                                            isOptionEqualToValue={(option, value) => option.rawMaterialName === value.rawMaterialName}
                                            ...
                                            renderInput={(params) => (
                                                <TextField {...params} error={formDataError.rawMaterialId} label="products" placeholder="Products" />
                                            )}
                                        />
                                    </div>
                                </div>
...
export default EditFactorySupplierInOut;