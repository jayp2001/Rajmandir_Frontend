...
function StockOutByCategoryStockInMaterialInOut() {
    ...
    const getStockOutDataOnPageChange = async (pageNum, rowPerPageNum) => {
        await axios.get(`${BACKEND_BASE_URL}rawMaterialrouter/getStockOutDataByCategory?page=${pageNum}&numPerPage=${rowPerPageNum}&categoryId=${categoryId}`, config)
            .then((res) => {
    ...
    const getStockOutDataOnPageChangeByFilter = async (pageNum, rowPerPageNum) => {
        await axios.get(`${BACKEND_BASE_URL}rawMaterialrouter/getStockOutDataByCategory?startDate=${state[0].startDate}&endDate=${state[0].endDate}&page=${pageNum}&numPerPage=${rowPerPageNum}&categoryId=${categoryId}`, config)
            .then((res) => {
                setStockOutData(res.data.rows);
                setTotalRowsOut(res.data.numRows);
                setTotalStockOutPrice(res.data.totalStockOutPrice)
    ...
    const getStockOutData = async () => {
        await axios.get(`${BACKEND_BASE_URL}rawMaterialrouter/getStockOutDataByCategory?page=${page + 1}&numPerPage=${rowsPerPage}&categoryId=${categoryId}`, config)
            .then((res) => {
                setStockOutData(res.data.rows);
                setTotalRowsOut(res.data.numRows);
                setTotalStockOutPrice(res.data.totalStockOutPrice)
    ...
    const getStockOutDataByFilter = async () => {
        await axios.get(`${BACKEND_BASE_URL}rawMaterialrouter/getStockOutDataByCategory?startDate=${state[0].startDate}&endDate=${state[0].endDate}&page=${1}&numPerPage=${rowsPerPage}&categoryId=${categoryId}`, config)
            .then((res) => {
                setStockOutData(res.data.rows);
                setTotalRowsOut(res.data.numRows);
                setTotalStockOutPrice(res.data.totalStockOutPrice)
    ...
    const stockInExportExcel = async () => {
        if (window.confirm('Are you sure you want to export Excel ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}rawMaterialrouter/exportExcelSheetForStockin?startDate=${state[0].startDate}&endDate=${state[0].endDate}` : `${BACKEND_BASE_URL}rawMaterialrouter/exportExcelSheetForStockin?startDate=${''}&endDate=${''}`,
                method: 'GET',
    ...
    const stockOutExportExcel = async () => {
        if (window.confirm('Are you sure you want to export Excel ... ?')) {
            await axios({
                url: filter ? `${BACKEND_BASE_URL}rawMaterialrouter/exportExcelSheetForStockOutDataByCategoryId?startDate=${state[0].startDate}&endDate=${state[0].endDate}&categoryId=${categoryId}` : `${BACKEND_BASE_URL}rawMaterialrouter/exportExcelSheetForStockOutDataByCategoryId?startDate=${''}&endDate=${''}&categoryId=${categoryId}`,
                method: 'GET',
                headers: { Authorization: `Bearer ${userInfo.token}` },
                responseType: 'blob', // important
            }).then((response) => {
...
export default StockOutByCategoryStockInMaterialInOut;