...
function MaterialListTableInOut() {
    const regex = /^\d*(?:\.\d*)?$/;
    ...
    const handleViewDetail = (id, name, unit, remainingQty, status) => {
        navigate(`/inOut/material/materialDetails/${id}/${name}/${unit}/${remainingQty}/${status == 0 || !status ? false : true}`)
    }
    if (loading) {
        console.log('>>>>??')
...
export default MaterialListTableInOut;
