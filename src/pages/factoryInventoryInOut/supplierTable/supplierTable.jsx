...
function FactorySupplierTableInOut() {
    ...
    const handleSupplierOnClick = (id) => {
        navigate(`/inOut/material/supplierDetails/${id}`)
    }
                    ...
                    <div className='userTableSubContainer'>
                        ...
                        <div className='grid grid-cols-12'>
                            ...
                            <div className='col-span-4 col-start-9 pr-8 flex justify-end'>
                                <button className='exportExcelBtn' onClick={() => { navigate(`/inOut/material/addSupplier`) }}>Add Supplier</button>
                            </div>
                        </div>
                                        ...
                                        {data?.map((row, index) => (
                                            totalRows !== 0 ?
                                                <TableRow
                                                    ...
                                                    className='tableRow'
                                                >
                                                    <TableCell align="left" onClick={() => handleSupplierOnClick(row.rmSupplierId)} >{(index + 1) + (page * rowsPerPage)}</TableCell>
                                                    <TableCell onClick={() => handleSupplierOnClick(row.rmSupplierId)} component="th" scope="row">
                                                        {row.supplierNickName}
                                                    </TableCell>
                                                    <TableCell onClick={() => handleSupplierOnClick(row.rmSupplierId)} align="left" >{row.supplierFirmName}</TableCell>
                                                    <TableCell onClick={() => handleSupplierOnClick(row.rmSupplierId)} align="left" >{row.supplierName}</TableCell>
                                                    <TableCell onClick={() => handleSupplierOnClick(row.rmSupplierId)} align="left" >{row.supplierPhoneNumber}</TableCell>
                                                    {/* <TableCell align="left" >{row.rightsName}</TableCell> */}
                                                    <TableCell onClick={() => handleSupplierOnClick(row.rmSupplierId)} align="left" >{parseFloat(row.remainingAmount ? row.remainingAmount : 0).toLocaleString('en-IN')}</TableCell>
                                                    <TableCell align="right">
                                                        <Menutemp rmSupplierId={row.rmSupplierId} handleOpen={handleOpen} data={row} deleteSupplier={handleDeleteSupplier} />
                                                    </TableCell>
                                                </TableRow> :
                                                <TableRow
                                                    key={row.userId}
...
export default FactorySupplierTableInOut;